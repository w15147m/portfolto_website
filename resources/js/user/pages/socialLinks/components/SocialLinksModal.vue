<template>
                        ...
                        <div class="">
                            <label class="block">
                                ...
                                <span class="relative mt-1.5 flex">
                                    <input v-model="form.platform" class="form-input peer w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 pl-9" placeholder="platform" type="text">
                                ...
                                <span>url</span>
                                <span class="relative mt-1.5 flex">
                                    <input v-model="form.url" class="form-input peer w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 pl-9" placeholder="url" type="text" />
                                    <span class="pointer-events-none absolute flex h-full w-10 items-center justify-center text-slate-400">
    ...
    const submitForm = () => {
        ...
        if (editMode.value) {
            funcApi.put(`/api/social-links/${form.value.id}`, form.value)
                .then((response) => {
                    const updatedExperience = response.data.socialLink;
            ...
            funcApi.post('/api/social-links', form.value)
                .then((response) => {
                    const NewAddExperience = response.data.socialLink;
